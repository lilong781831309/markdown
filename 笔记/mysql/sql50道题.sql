...
################################
SELECT a.s_id,a.s_name,IFNULL(ROUND(AVG(b.s_score),2),0) AS avg_score
FROM student a
LEFT JOIN score b ON a.s_id = b.s_id
GROUP BY a.s_id,a.s_name
...
INNER JOIN teacher d ON c.t_id = d.t_id AND d.t_name = '张三';
...
INNER JOIN  teacher ON course.t_id = teacher.t_id AND t_name = '张三'
)tmp
...
EXPLAIN
SELECT a.* FROM student a
INNER JOIN (SELECT c_id FROM score WHERE s_id = '01') b
...
GROUP BY a.s_id,a.s_name,c.c_name
...
SELECT
  ...
  b.avg_score AS 平均分,
  b.pass_rate AS 及格率,
  b.medium_rate AS 中等率,
  b.fine_rate AS 优良率,
  b.excellence_rate AS 优秀率
FROM course a
JOIN(
  SELECT 
    c_id,
    ...
    SUM(IF(s_score >= 60,1,0))/COUNT(1) AS pass_rate,
    SUM(IF(s_score >= 70 AND s_score < 80,1,0))/COUNT(1) AS medium_rate,
    SUM(IF(s_score >= 80 AND s_score < 90,1,0))/COUNT(1) AS fine_rate,
    SUM(IF(s_score >= 90,1,0))/COUNT(1) AS excellence_rate
  FROM score
  GROUP BY c_id
)b
ON a.c_id = b.c_id;
...
SELECT 
  ...
  SUM(IF(a.s_score >= 60,1,0))/COUNT(1) AS 及格率,
  SUM(IF(a.s_score >= 70 AND a.s_score < 80,1,0))/COUNT(1) AS 中等率,
  SUM(IF(a.s_score >= 80 AND a.s_score < 90,1,0))/COUNT(1) AS 优良率,
  SUM(IF(a.s_score >= 90,1,0))/COUNT(1) AS 优秀率
FROM score a
